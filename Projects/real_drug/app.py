...
client = MongoClient('localhost', 27017) 
db = client.drug                   
#crawl_________________________________________________
import requests
from bs4 import BeautifulSoup
...
def test_post():
   # url = 'https://terms.naver.com/entry.nhn?docId=2136613&cid=51000&categoryId=51000'
...
#    pre_name = soup.select_one ('#content > div.section_wrap > div.headword_title > h2')
#    name = pre_name.text.replace('/h','')

#    #성분/ #size_ct > p:nth-child(8)/ 결과값: <p class="txt">아세트아미노펜 300mg, 이소프로필안티피린 150mg, 카페인무수물 50mg</p>
#    pre_ingrt = soup.select_one ('#size_ct > p:nth-child(8)')
#    ingrt = pre_ingrt.text.replace('p','')

#    #이미지/ '#innerImage0' 속 'data-src'/  결과값: https://dbscthumb-phinf.pstatic.net/3323_000_18/20200419235410005_9M2AIXW4V.jpg/A11A1270A006002.jpg?type=m250&wm=N
#    pre_img_url = soup.select_one ('#innerImage0')['data-src']
...
#    pre_effect = soup.select_one ('#size_ct > p:nth-child(14)')
#    effect = pre_effect.text.replace('/p','')
...
#    pre_intrc= soup.select_one ('#size_ct > p:nth-child(16)')
#    intrc = pre_intrc.text.replace('/p','')

#    #size_ct > p:nth-child(16)
